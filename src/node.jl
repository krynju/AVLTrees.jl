...
function Node{K,V}(key::K, data::V, parent::Union{Node{K,V},Nothing}) where {K,V}
    return Node{K,V}(parent, nothing, nothing, key, Int8(0), data)
end
Node{K,V}(key::K, data::V) where {K,V} = Node{K,V}(key, data, nothing)
